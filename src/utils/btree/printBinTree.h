...
public:
  ...
  void insert() {}
  template <typename... Args> void insert(const T &val, Args... more) {
    if (!root) {
      root = new Node(val);
    } else {
      Node *p = root;
      for (;;) {
        if (val == p->val)
          return;
        Node *&pchild = val < p->val ? p->left : p->right;
        if (!pchild) {
          pchild = new Node(val);
          break;
        }
  ...
  display_rows get_row_display() const {
    ...
    // its Node's val, created using a std::stringstream object.
  ...
  vector<string> row_formatter(const display_rows &rows_disp) const {
    ...
    for (s_t r = 0; r < row_count; ++r) {
      ...
      // A row of spaced-apart val strings is ready, add it to the result
      // vector
      ...
      // to "connect" two rows' Node val strings.
  ...
  void Dump() const {
    ...
    // This tree is not empty, so get a list of node vals...
